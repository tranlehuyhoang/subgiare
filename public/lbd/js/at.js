console.log("%c Website: SubGiaRe.Vn %c", 'font-family: "Helvetica Neue", Helvetica, Arial, sans-serif;font-size:24px;color:#00bbee;-webkit-text-fill-color:#00bbee;-webkit-text-stroke: 1px #00bbee;', "font-size:12px;color:#999999;");
console.log("%c Coder: Hoàng Văn Lĩnh %c", 'font-family: "Helvetica Neue", Helvetica, Arial, sans-serif;font-size:24px;color:#00bbee;-webkit-text-fill-color:#00bbee;-webkit-text-stroke: 1px #00bbee;', "font-size:12px;color:#999999;");
console.log("%c Contact: https://www.facebook.com/hvl2k3.it/ %c", 'font-family: "Helvetica Neue", Helvetica, Arial, sans-serif;font-size:24px;color:#00bbee;-webkit-text-fill-color:#00bbee;-webkit-text-stroke: 1px #00bbee;', "font-size:12px;color:#999999;");
...
function statusOrder(status) {
    ...
    switch (status) {
        case "Success":
            msg = '<button type="button" class="btn btn-success btn-sm">Đã hoàn thành</button>';
            break;
        case "Pause":
            msg = '<button type="button" class="btn btn-danger btn-sm">Đã hủy</button>';
            break;
        case "Report":
            msg = '<button type="button" class="btn btn-warning btn-sm">Tạm dừng</button>';
            break;
        case "Refund":
            msg = '<button type="button" class="btn btn-success btn-sm">Đã hoàn tiền</button>';
            break;
        case "Waiting":
            msg = '<button type="button" class="btn btn-warning btn-sm">Chờ hủy</button>';
            break;
        case "Expired":
            msg = '<button type="button" class="btn btn-danger btn-sm">Đã hết hạn</button>';
            break;
        case "CookieDie":
            msg = '<button type="button" class="btn btn-danger btn-sm">Cookie die</button>';
            break;
        case "ProxyError":
            msg = '<button type="button" class="btn btn-warning btn-sm">Proxy lỗi</button>';
            break;
        default:
            msg = '<button type="button" class="btn btn-info btn-sm">Đang hoạt động</button>';
            break;
    }
    return msg;
}
...
function igGetUsername(elm) {
    ...
    setTimeout(() => {
        ...
        if (username) {
            username = username[1];
            $("[name=" + elm + "]").prop("disabled", false).val(username);
        } else {
...
function igGetIdpost(elm) {
    ...
    setTimeout(() => {
        ...
        if (idpost) {
            idpost = idpost[1];
            $("[name=" + elm + "]").prop("disabled", false).val(idpost);
        } else {
            $("[name=" + elm + "]").prop("disabled", false).val("");
        }
        $("#buy").prop("disabled", false);
